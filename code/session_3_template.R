## Programming for Professional Research Using R -- Session 3
  ...
  european_data <- european_data %>%
      ...
      dplyr::mutate(
          ...
          dplyr::across( # Across is magic and allows you to modify multiple variables at once
              ...
              matches("^Q0[1-6]"), # Same as for select() above. IMPORTANT -- all variables need to be
                                   # of the same 'type' (e.g. character, numeric)
              
              ~ case_when( # across() basically loops over every relevant variable. '.x' refers to each
                           # variable being treated
                  ...
                  TRUE                      ~ .x # We've created NAs and are leaving the other values
                                                 # alone
...
# group_by() and summarize()
  ...
  european_country_data <- european_data %>% # New observation level so new dataset
      ...
      dplyr::summarize(
          ...
          # Summarize aggregated values based on what we instruct it to do. If we didn't use
          # group_by(), it would summarize to one single value. Here, it will output one value
          # per country.
      ...
      dplyr::ungroup() # Always remember to do this! Otherwise your future code will do weird things
  
    ### Step 5 -- Create a question-level dataset ----
  
# The above is useful. But what if we want to sum up countries' values for all of these questions, or
# look at each country's 'average' enthusiasm, it'll be easier with a 'long' dataset than a 'wide' one.
  
  european_country_data_wide <- european_country_data %>%
      
      tidyr::pivot_longer(
          
          cols      = matches("^Q0[1-6]"), # Variables whose data we want to be in a single, 'long' variable
          
          names_to  = "topic", # Creates a variable named 'topic' that saves the variable names
          
          values_to = "score" # Creates a 'long' variable named 'score' that holds all of the original values
...
# Now for example I can look at average 'enthusiasm' by country
  ...
  average_country_enthusiasm <- european_country_data_wide %>%
      ...
      dplyr::arrange(average_score) # Order them from highest to lowest enthusiasm (NOTE -- smaller
...
## This will show you some basic tasks that you can do with raw data to remove possible errors prior to
## analysis:
  
  ## Check for duplicate observations -> i.e. rows should be uniquely identified
  ## Encode missing values -> answers such as "don't know" or "refused to respond" shouldn't be negative but NA
  ## Check and confirm numerical outliers (not covered here)
  
    ### Step 1 -- Check for duplicate observations ----
  
# In the norms_values_data dataset, the unique identifier for each observation is "D_INTERVIEW". It
# should be the case that each ID is only used once in the dataset. To check for duplicates, this is
# the preferred method:
  ...
  duplicates <- norms_values_data %>%
      ...
      # Grouping at the ID level, we'll be able to check how many times it shows up
      
      dplyr::summarize(
          
          num = n()
          
          # n() counts the number of instances of each ID! Just run up to summarize() and add %>%
          # head() to see what it does
          
      ) %>%
      
      filter(num > 1)
  
# 36 duplicates, uh oh. They all seem to be similar in "order" so I'll check in which countries they
# occurred:
  
  duplicates <- duplicates %>%
      
      left_join( # left_join() allows us to 'merge' datasets, in this case merging norms_values_data
                 # into the duplicates dataset to add countries
          
          norms_values_data %>%
              
              select(D_INTERVIEW, B_COUNTRY_ALPHA) %>% # Need to have a common variable (D_INTERVIEW)
                                                       # between norms_values_data and duplicates, and
                                                       # we only want to add country name
              
              distinct() # Need to do this BECAUSE of the duplicate IDs
          
      )
  
# NOTE -- "left_join" means that only the observations from the first (left) dataset (based on value of D_INTERVIEW),
# here "duplicates", are kept. e could have used "right_join" if we wanted to only keep the observations from the
# second (right) dataset, here "norms_values_data", "full_join" if we wanted to keep both, or "inner_join" if we only
# wanted to keep observations that appear in both datasets
...
# There is an issue with the Mongolian IDs. Usually we would have to contact the data collection team to determine
# what the problem is. In this case we can't do that, so... Just remove all observations from Mongolia because this
# is strange
...
# 'Easy' selection method would be this:
  ...
  child_data <- norms_values_data %>%
      ...
      select(
          ...
          D_INTERVIEW, B_COUNTRY_ALPHA, Q07_child_manners, Q08_child_independence, Q09_child_hard_work,
          
          Q10_child_responsibility, Q11_child_imagination, Q12_child_tolerance, Q13_child_thrift,
          
          Q14_child_determined, Q15_child_faith, Q16_child_unselfish, Q17_child_obedient # Ugh
...
# These negative values don't seem great. If we look at the codebook, we see that they're different versions
# of not receiving a quantifiable answer (e.g. don't know, refused to respond). We need to encode those as NA
  ...
  child_data_sucky_mutate <- child_data %>%
      ...
      mutate(
          ...
          Q07_child_manners = case_when(
              ...
              Q07_child_manners < 0 ~ NA_integer_, # Notice that they're always negative so this is an easy simplification
...
# That sucked. Look how much easier the below is:
  ...
  child_data <- child_data %>%
      ...
      ) # Same result!!! You can compare child_data and child_data_sucky_mutate to see that
  
# Note -- in this dataset, '1' is that the subject was mentioned, '2' that the subject wasn't. This is akin to
# 'yes' and 'no' which are traditionally encoded respectively as '1' and '0'. So:
      ...
      mutate(
          ...
          across(
              ...
              ~ case_when(
                  ...
                  TRUE    ~ as.numeric(.x) # Note that we need to change class from integer to numeric because of the '0'
...
# In this case, it seems that the answers to the question of what is important to teach to a child is spread
# across multiple variables, one for each answer basically. This would make it difficult to e.g. check what the
# most popular 5 answers are. Solution: pivot_longer
  
  child_data_long <- child_data %>% # Note -- good to create a new dataset when modifying the level of observation
      ...
      pivot_longer(
          ...
          names_prefix = "Q[0-9]{2}_child_", # Allows us to only keep what follows the variable prefix
...
# Challenge: Try to determine what the five most popular answers to this question were. Hint: "arrange(-var)" orders
# a dataset from the largest value of 'var' to the small value of 'var'. Solution below.
  ...
  popular_answers <- child_data_long %>%
      ...
      dplyr::summarize(
          ...
          num = sum(child_quality_value, na.rm = TRUE)
